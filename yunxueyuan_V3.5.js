...
(function () {
    ...
    var objURL = {
        ...
        , insert: function (data_add) {
            Array.prototype.insert = function (index, value) {
                this.splice(index, 0, value);
            }
            this.aurls.insert(this.courseN, data_add);
        }
        , url: function () {
            if (this.courseN < 0) {
                console.log("错误：已经为第一个");
    ...
    var OURL = Object.create(objURL);
    // 视频动作管理
    var objVideo = {
        myvideo: new Object
        , playing: false
        , getVideo: function () {
            var video = $("video.vjs-tech")[0];//单页Video获取
            //判断是否解析到视频
            // if (video == undefined) {
            //     video = scormIframe.contentWindow.$("video")[0];//单页三层Video获取，未完成，解除注释后父子框架出现连续报错
            // }
            if (video == undefined) {
                video = $("iframe#myFrame").contents().find("video")[0]; // 框架下正常video获取
            }
            if (video == undefined) {
                video = myFrame.contentWindow.$("iframe")[0].contentWindow.scormIframe.contentWindow.$("video")[0]; // 获取iframe下三层的video
            }
            if (video == undefined) {
        ...
        , play: function () {
            ...
            if (this.playing == false) {
                if (!$("iframe#myFrame").contents().find("button.videojs-referse-btn").is(":hidden")) { $("iframe#myFrame").contents().find("button.videojs-referse-btn")[0].click(); }
                if (!$("iframe#myFrame").contents().find("img.register-img").is(":hidden")) {
            ...
            // if (this.playing == false) {
            //     if (!$("iframe#myFrame").contents().find("button.videojs-referse-btn").is(":hidden")) { $("iframe#myFrame").contents().find("button.videojs-referse-btn")[0].click(); }
            //     if (!$("iframe#myFrame").contents().find("img.register-img").is(":hidden")) {$("iframe#myFrame").contents().find("img.register-img")[0].click();}
            //     this.myvideo.play();
            // }
            // this.showProgress();
        }
        ...
        , removeWaterMark: function () {
            ...
            $("iframe#myFrame").contents().find("div.vjs-marquee").remove();//移除顶部黑底浮窗
        }
        , isFinished: function () {
    ...
    var objFrame = {
        ...
        , getRegister: function () {
            // 获取register图标
            var register = $("iframe#myFrame").contents().find("img.register-img")[0];
    ...
    if (localurl.indexOf('/oauth/#login') != -1) { // 登入页面
        ...
        //GM_addStyle('.sidebar-toolbar {display:none}');
        // 添加按钮container
        var isFirst = false; // 是否已经存在子页面的button
        var isTop = false; //是否是顶部框架
        if (top.document.getElementById("myFrame") == null) { isTop = true }


        var obj = top.document.getElementById("xxxnav");
        if (obj == null) {
            obj = document.createElement("div");
            obj.id = 'xxxnav';
            obj.style = "position: fixed;top:0;right:0px;z-index:9999;width:0px;height:100%";
            var odiv0 = document.createElement("div");
            odiv0.id = "button-container";
            odiv0.className = "button-container";
            obj.appendChild(odiv0);
            document.body.appendChild(obj);
            isFirst = true;
        }
        var odiv = obj.firstChild;
        if ($(odiv).find("#remove_barrage")[0] == null) {
            isFirst = true;
        }
// 从未添加过子页面按钮
        if (isFirst) {
...
`);
            // 添加按钮
            // 功能1：去除水印
            createButton(odiv, 'remove_barrage', 'button1', '1.移除水印')
            if (isTop) {
                $("#remove_barrage")[0].onclick = function () {
                    $("div.vjs-barrage").remove(); //移除随机浮动浮窗
                    $("div.vjs-marquee").remove();//移除顶部黑底浮窗
                    //objVideo.removeWaterMark();
                }
            }
            else {
                $("#remove_barrage", window.parent.document)[0].onclick = function () {
                    //$("iframe#myFrame").contents().find("div.vjs-barrage").remove(); //移除随机浮动浮窗
                    //$("iframe#myFrame").contents().find("div.vjs-marquee").remove();//移除顶部黑底浮窗
                    objVideo.removeWaterMark();
                }
            }
            ...
            // 功能2：视频加速
            createButton(odiv, 'speed_up', 'button1', '2.加速')
            if (isTop) {
                $("#speed_up")[0].onclick = function () {
                    objVideo.getVideo();
                    var myvideo = objVideo.myvideo;// 获取video节点
                    myvideo.playbackRate = myvideo.playbackRate + 0.1;
                    if ($("#current_playbackRate")[0] == undefined) {
                        // // 插入元素
                        var oinp = document.createElement("input");
                        oinp.id = 'current_playbackRate';
                        oinp.className = "ele_input";
                        oinp.value = myvideo.playbackRate;
                        odiv.appendChild(oinp);
                    }
                    else { // 刷新当前速度值
                        $("#current_playbackRate")[0].value = myvideo.playbackRate;
                    }
                }
            }
            else {
                $("#speed_up", window.parent.document)[0].onclick = function () {
                    objVideo.getVideo();
                    var myvideo = objVideo.myvideo;// 获取video节点
                    myvideo.playbackRate = myvideo.playbackRate + 0.1;
                    if ($("#current_playbackRate", window.parent.document)[0] == undefined) {
                        // // 插入元素
                        var oinp = document.createElement("input");
                        oinp.id = 'current_playbackRate';
                        oinp.className = "ele_input";
                        ...
                        $("#current_playbackRate", window.parent.document)[0].value = myvideo.playbackRate;
                    }
                }
            }
            // 功能3：视频减速
            createButton(odiv, 'speed_down', 'button1', '3.减速')
            if (isTop) {
                $("#speed_down")[0].onclick = function () {
                    objVideo.getVideo();
                    var myvideo = objVideo.myvideo;
                    myvideo.playbackRate = myvideo.playbackRate - 0.1;
                    // 显示当前速度
                    if ($("#current_playbackRate")[0] == undefined) {
                        // // 插入元素
                        ...
                        oinp.className = "ele_input";
                        oinp.value = myvideo.playbackRate;
                        //odiv.insertBefore(oinp, $("#remove_barrage")[0]);
                        odiv.appendChild(oinp);
                    }
                    else { // 刷新当前速度值
                        $("#current_playbackRate")[0].value = myvideo.playbackRate;
                ...
                $("#speed_down", window.parent.document)[0].onclick = function () {
                    objVideo.getVideo();
                    var myvideo = objVideo.myvideo;// 获取video节点
                    myvideo.playbackRate = myvideo.playbackRate - 0.1;
                    if ($("#current_playbackRate", window.parent.document)[0] == undefined) {
